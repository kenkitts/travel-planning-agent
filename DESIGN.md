...
| 11 | AgentCore Runtime V2 (`platformVersion`) | The new Runtime platform version (announced 2026-09-18) — starts each instance from a prepared snapshot instead of a cold boot, giving consistent ~2s P75 cold starts regardless of image size/concurrency, and bills for memory actually used (reclaimed as it goes cold) instead of the session's peak. Set via a `platformVersion: "V2"` field on `create`/`update`-`AgentRuntime`. | Directly relevant to this project's own architecture — `RuntimeStack` bundles `agent/` into a container image, and cold-start latency/cost is a real, previously-unaddressed axis this project has never optimized. **Confirmed blocked, not just untried**: AWS's own docs state "AWS CloudFormation and the AWS CDK do not currently support setting `platformVersion`" — independently verified by inspecting the installed `aws_cdk.aws_bedrockagentcore.CfnRuntime` construct's real `__init__` signature directly, which has no `platform_version` parameter in either direction (create or update). The only way to set it today is a direct `boto3`/CLI call outside CDK, which this project's CDK-only, no-drift convention (decision #11, decision #125) rejects as a standing practice unless wrapped in a custom resource (`AwsCustomResource`) — a real chunk of added complexity for a change that's currently pure cost/latency optimization with no functional benefit at this project's actual traffic volume (decision #137's real numbers: low, non-bursty). Also has real migration considerations to verify before adopting even via a custom resource: V2 create/update takes minutes (snapshot preparation) instead of seconds and requires `/ping` to report healthy within 120 seconds of startup; V2 currently caps total environment-variable size at 2.5 KB for container agents (vs. 4 KB on V1) — this project's Runtime already sets `GATEWAY_URL`/`MEMORY_ID`/`GUARDRAIL_ID`/`GUARDRAIL_VERSION`/`GATEWAY_INFERENCE_URL`/`GATEWAY_OBO_PROVIDER_NAME`/`MODEL_ID`/`AWS_REGION`, worth a real byte-size check before assuming it clears that budget; and AWS's own "Optimize your agent for AgentCore Runtime V2" migration guide (linked from the launch blog) could not be loaded at ideation time (the URL redirected to AgentCore's generic overview page, suggesting it isn't published yet or the link is stale) — its actual code-level guidance should be read directly before writing any V2-specific agent code, not assumed from the blog's summary alone. Revisit once either CDK/CloudFormation ships native support, or this project's real traffic justifies the custom-resource complexity. |
